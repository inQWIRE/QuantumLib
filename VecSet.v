...
Lemma get_minor_mul_0 : forall {n} (A : Square (S n)) (v : Vector (S n)),
  get_vec 0 A = @e_i (S n) 0 -> (get_minor A 0 0) × (reduce_row v 0) = reduce_row (A × v) 0.
Proof. intros. 
       prep_matrix_equality. 
       unfold Mmult, get_minor, reduce_row. 
...
Lemma reduce_mul_n : forall {n} (A : Square (S n)) (v : Vector (S n)),
  get_vec n A = @e_i (S n) n -> (get_minor A n n) × (reduce_row v n) = reduce_row (A × v) n.
Proof. intros. 
       prep_matrix_equality. 
       unfold Mmult, get_minor, reduce_row.
...
(* here is a generalization of the above two lemmas *)
...
Proof. intros.
       ...
       bdestruct (x <? i).
       - rewrite <- get_vec_conv, Nat.add_0_r, H0.
...
Proof. intros. 
       prep_matrix_equality.
       unfold get_minor, pad1, col_wedge, row_wedge, e_i. 
       bdestruct_all.
